...
async fn perform_form_login(request: LoginRequest, state: State<'_, ProxyState>) -> Result<LoginResponse, String> {
    ...
    println!("[perform_form_login] Logging in to: {}", login_url);
    ...
    println!("[perform_form_login] Response status: {} (success: {})", status, success);
                    ...
                    match scraper::Selector::parse(&selector) {
                        Ok(css_selector) => {
                            ...
                            if extracted.is_empty() {
                                ...
                                println!("[perform_form_login] Extracted text: {}", extracted.trim());
                                Some(extracted.trim().to_string())
                            }
                        }
                        Err(e) => {
                            println!("[perform_form_login] Invalid CSS selector '{}': {:?}", selector, e);
                            None
                        }
                    }
                }
